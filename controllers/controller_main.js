...
async function fetchRegisterNo(req, res){
  ...
  console.log('Received data from vb.net ' + Register_No)
  if (!{Register_No}) {
    return res
      .status(400)
  ...
  try {
    ...
    const data = await db.executeStoredProcedureWithParams(
      Register_No
    );
...
async function executeStoredProcedure(req, res, Register_No) {
  try {
    ...
    if (!Register_No || typeof Register_No !== "string") {
      return res.status(400).send({ message: "Invalid or missing Register_No" });
    }
    ...
    if (!result || result.length === 0) {
      return res.status(404).send({ message: "No found for the given Register_No" });
    }
    // Return a success response with the result
    res.status(200).send({ success: true, data: result });
  } catch (error) {
    console.error(`Error executing stored procedure: ${error.message}`);
    res.status(500).send({ message: "Error executing stored procedure", error: error.message });
  }
}
